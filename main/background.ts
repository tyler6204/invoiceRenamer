...
// Resolve the path to the .env file depending on whether the app is packaged
...
// We'll log this after the logging setup is complete
let envLoadResult = { parsed, error };
...
// Log environment variable loading result
if (envLoadResult.error) {
  logToFile(`dotenv error: ${envLoadResult.error}`);
}
logToFile(`GH_TOKEN loaded: ${process.env.GH_TOKEN ? 'YES' : 'NO'}`);
...
(async () => {
  ...
  try {
    ...
    mainWindow.webContents.loadURL(
      `data:text/html,<html><body><h1>Failed to load app</h1><pre>${error.message}</pre></body></html>`
    );
  }
...
ipcMain.handle("open-file", async (_, filePath) => {
  ...
  try {
    if (
      ...
      logToFile(
        `[open-file] Failed: Path is not absolute or invalid: "${filePath}"`
      );
      return {
        success: false,
        error: "Cannot open file: Path must be absolute and valid.",
      };
    }
...
ipcMain.handle("check-file-exists", async (_, filePath) => {
  ...
  try {
    if (
      ...
      logToFile(
        `[check-file-exists] Failed: Path is not absolute or invalid: "${filePath}"`
      );
      return {
        exists: false,
        error: "Path must be absolute for existence check",
      };
    }
    await fs.promises.access(filePath, fs.constants.F_OK);
    ...
    logToFile(
      `[check-file-exists] Error checking file existence for "${filePath}": ${error.message}`
    );
...
ipcMain.handle(
  ...
  async (
    ...
    desiredNewBaseName: string,
    fileExtension: string
  ) => {
    logToFile(
      `[resolve-and-rename] Request: original="${originalFilePath}", desiredBase="${desiredNewBaseName}", ext="${fileExtension}"`
    );
    ...
    try {
      ...
      if (
        ...
        !path.isAbsolute(originalFilePath)
      ) {
        throw new Error(
          `Invalid originalFilePath: Must be an absolute path. Received: "${originalFilePath}"`
        );
      }
      if (!desiredNewBaseName || typeof desiredNewBaseName !== "string") {
        throw new Error(
          `Invalid desiredNewBaseName. Received: "${desiredNewBaseName}"`
        );
      }
      if (typeof fileExtension !== "string") {
        // Allow empty extension, but must be string
      ...
      if (fileExtension.length > 0 && !fileExtension.startsWith(".")) {
        logToFile(
          `[resolve-and-rename] Warning: fileExtension "${fileExtension}" does not start with '.', adding it.`
        );
      ...
      while (exists) {
        ...
        try {
          ...
          if (error.code === "ENOENT") {
            logToFile(
              `[resolve-and-rename] Path does not exist. Final path determined.`
            );
            exists = false; // Exit loop, path is free
          } else {
            // Rethrow other errors (e.g., permissions)
            throw new Error(
              `Error checking existence for "${fullNewPath}": ${error.message}`
            );
          }
        }
        ...
        if (counter > 100) {
          // Safety break
          throw new Error(
            "Too many conflicts detected while resolving rename path."
          );
        }
      } // End while loop
      ...
      logToFile(
        `[resolve-and-rename] CONFIRMING rename call: fs.rename("${originalFilePath}", "${fullNewPath}")`
      );
      ...
      logToFile(
        `[resolve-and-rename] Failed. Error: ${
          error.message
        }. Original: "${originalFilePath}", Target Attempt: "${
          fullNewPath || "unknown"
        }"`
      ); // Log target attempt too
      return {
        success: false,
        error: error.message,
        originalPath: originalFilePath,
        attemptedPath: fullNewPath,
...
ipcMain.handle(
  "resolve-and-duplicate",
  async (_, sourcePath: string, desiredTargetPath: string) => {
    logToFile(
      `[resolve-and-duplicate] Request: source="${sourcePath}", desiredTarget="${desiredTargetPath}"`
    );
    try {
      // --- Validation ---
      if (
        ...
        throw new Error(
          `Invalid sourcePath: Must be an absolute path. Received: "${sourcePath}"`
        );
      }
      if (
        !desiredTargetPath ||
        typeof desiredTargetPath !== "string" ||
        !path.isAbsolute(desiredTargetPath)
      ) {
        throw new Error(
          `Invalid desiredTargetPath: Must be an absolute path. Received: "${desiredTargetPath}"`
        );
      }
      ...
      try {
        ...
        logToFile(
          `[resolve-and-duplicate] Source path exists and is readable.`
        );
      } catch (err: any) {
        if (err.code === "ENOENT") {
          ...
          throw new Error(
            `Cannot access source file "${sourcePath}": ${err.message}`
          );
        }
      }
      ...
      try {
        ...
        logToFile(
          `[resolve-and-duplicate] Target path "${desiredTargetPath}" already exists. Aborting duplicate.`
        );
        ...
        if (error.code === "ENOENT") {
          ...
          logToFile(
            `[resolve-and-duplicate] Target path does not exist. Proceeding with copy.`
          );
        } else {
          // Other error checking target
          throw new Error(
            `Error checking target path "${desiredTargetPath}": ${error.message}`
          );
        }
      }
      ...
      logToFile(
        `[resolve-and-duplicate] Attempting copy: "${sourcePath}" -> "${desiredTargetPath}"`
      );
      ...
      logToFile(
        `[resolve-and-duplicate] Failed. Error: ${error.message}. Source: "${sourcePath}", Target: "${desiredTargetPath}"`
      );
      return {
        success: false,
        error: error.message,
